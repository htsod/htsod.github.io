...
author: ["Max"]
draft: true
bibfile: "./static/group_theory/citation.json"
katex: true
math: true
description: ""
showToc: true
...
## Mathematical Prelimary

### Group Theory 

Group theory is the study of symmetry. For a equilateral triangle on 2-dimensional plane. Rotation by \\(R_{1}= 2\pi/3\\), \\(R_{2}=4\pi/3\\) and \\(I = 2\pi\\) as well as relfection \\(r_{1}\\) and \\(r_{2}\\) will leave the equilateral triangle unchanged. If we put them in a set, \\(\{ I, R_{1}, R_{2}, r_{1}, r_{2} \}\\), it forms a group $G$ with group element $g_{i}$. Group satisfies closure, any group element $g_{i}$ multiply with any group element $g_{j}$ will belong to the group. 

$$ g_{i} g_{j} = g_{k} $$

For the equilateral triangle, two rotation becomes the identity or another rotation which belongs to the group. Two consecutive reflection will give a rotation or identity which also belongs to the group. Closure is satisfy for the equilateral triangle.

The multiplication is associative, but not necessarily commutative. Group also required an inverse and an identity which is also true for the triangle group.


### Representation Theory

Group theory by itself is abstract mathematical entity. To make it physical, one has to define a representation of the group that would preserve the multiplicative structure of the group.

$$ D(g_{i})D(g_{j}) = G(g_{k}) $$

where $g_{i},g_{j},g_{k}$ all belong to the group $G$.

A matrix, for example, could a representation. Then the form given above will be the multiplication between two matrices. Notice that matrix is associative but not commutative, which align with the property of group multiplication.

A more general form of a representation would be a tensor with arbitrary number of indice $n$. For example, an $n$ indice tensor $T^{i_{1}, i_{2},...,i_{n}}$ in $N$ dimensional space furnishes $N^{n}$ dimensional representation. For $n=1$ it is the vector representation, and for $n=2$ it will be the matrix representation.

Another useful representation is the basic function. For a group operation $\hat{P_{R}}$ acting on function $\psi_{1}$ labeled the irreducible representation $\Gamma_{n}$. It maps the function in a set of irreducible representation $\{\psi_{1}, \psi_{2},...,\psi_{n}\}$, where $n$ is the order of the group. In basic of the set, the operation $\hat{P_{R}}$ maps $\psi$ into linear combination of the sets, and hence it furnish an irreducible representation of the group.

One might stop and pounder what represention does a group have. Infinite! Imagining representing a point in three dimensional space. We could use the Cartesian coordinate $(x, y, z)$ or the polar coordinate $(r, \theta, \phi)$. The representaiton is not unique. Yet, we cannot reduce the number of parameters lower than three and representation that has a dimension higher than three will be reducible to three independent axis.

The representaiton in group theory can also be reducible or irreducible. It is the irreducible representation of group theory that gives rise to numerous orthogonality theorem that will serve a great important.

1. Unitary Representation

Compact group, which is defined to have a finite group sum \\( \int{du_{g}} \\), can be represented by unitary representation, \\( D^{\dagger}(g) D(g) = I \\). Recall from the axiom of qunatum mechanics, the state vector transform with a unitary operator. A first hint that these math entities could be connected in some way.


2. Schur's Lemma

Suppose a operator commute with all the group elements, the operator could be diagonalized and proportional to the identity matrix. When we sum over the group, something nice will happen, like the orthogonality theorem. 

The second part of Schur's Lemma

3. Great Orthogonality Theorem

The great orthogonality theorem states that when we sum over two irreducible representation of a group, it leaves behind a bunch of ones and zeros.

$$ \sum_{g}{D^{\Gamma}_{lk}(g)D^{\Gamma^{\prime}}_{l^{\prime}k^{\prime}}(g)}=\frac{l_{\Gamma}}{n} \delta_{\Gamma \Gamma^{\prime}} \delta_{} $$


2. Character is a function of class

Irrducible representation, as the name suggests, is unique up to a similarity transformation. Depending on the basis that we working with, it has different entries. But the character of the group, defined from the trace of the represenation, is built to be invariant under similarity transformation. Thus, it will be helpful to working with orthongonality in character than the orthogonality in representation.  
